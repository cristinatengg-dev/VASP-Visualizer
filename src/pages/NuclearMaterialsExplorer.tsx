...
import {
  NUCLEAR_DATABASES,
  type NuclearDatabaseCategory,
  type NuclearDatabaseIntegrationStage,
  type NuclearDatabaseRecord,
  type NuclearDatabaseTier,
} from '../data/nuclearDatabases';
...
type AccessFilter = 'all' | NuclearDatabaseTier;
type CategoryFilter = 'all' | NuclearDatabaseCategory;
...
const CATEGORY_META: Record<NuclearDatabaseCategory, { icon: LucideIcon; accent: string; chip: string }> = {
  Structure: {
    icon: Database,
...
const TIER_META: Record<NuclearDatabaseTier, { label: string; icon: LucideIcon; chip: string; panel: string }> = {
  open: {
    label: 'Open',
    icon: Globe2,
    chip: 'bg-emerald-50 border-emerald-200 text-emerald-700',
...
const STAGE_META: Record<NuclearDatabaseIntegrationStage, { label: string; chip: string }> = {
  connected: {
    label: 'Connected now',
...
const STAGE_PRIORITY: Record<NuclearDatabaseIntegrationStage, number> = {
  connected: 0,
  ready: 1,
  'metadata-first': 2,
  'apply-first': 3,
...
const categoryLabel = (value: CategoryFilter) => (value === 'all' ? 'All categories' : value);
...
const SourceCard: React.FC<{ source: NuclearDatabaseRecord }> = ({ source }) => {
...
const NuclearMaterialsExplorer: React.FC = () => {
  ...
  const handleViewStructure = (entry: MaterialEntry) => {
    ...
    params.set('prompt', `Build a nuclear materials starter model for ${entry.formula} (${entry.crystal_system}, ${entry.space_group || 'unknown SG'})`);
  ...
  }, [accessFilter, categoryFilter]);

  const readyNow = useMemo(
    () => NUCLEAR_DATABASES.filter((source) => source.integrationStage === 'connected' || source.integrationStage === 'ready'),
    [],
  );
  const applyFirst = useMemo(
    () => NUCLEAR_DATABASES.filter((source) => source.integrationStage === 'apply-first'),
    [],
  );
  const licenseFirst = useMemo(
    () => NUCLEAR_DATABASES.filter((source) => source.integrationStage === 'license-first'),
    [],
  );

  const sourceCounts = useMemo(
    () => ({
      open: NUCLEAR_DATABASES.filter((source) => source.tier === 'open').length,
      controlled: NUCLEAR_DATABASES.filter((source) => source.tier === 'controlled').length,
      commercial: NUCLEAR_DATABASES.filter((source) => source.tier === 'commercial').length,
      connected: NUCLEAR_DATABASES.filter((source) => source.integrationStage === 'connected').length,
    }),
    [],
  );
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-black tracking-wide text-[#0A1128]">Nuclear Materials Explorer</h1>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
            <div>
              ...
              <h2 className="max-w-3xl text-2xl font-black leading-tight text-[#0A1128]">
                One place for open nuclear structure search and the current map of controlled reactor-material data.
              </h2>
              <p className="mt-3 max-w-3xl text-sm leading-relaxed text-gray-500">
                The live search below uses the structure sources already wired into this repo today.
                The registry underneath separates what we can plug in immediately from what needs an account,
                consortium access, or a commercial license before we touch integration.
              </p>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
              <div className="rounded-[22px] border border-emerald-100 bg-emerald-50/70 p-4">
                ...
                <p className="mt-2 text-sm font-bold text-[#0A1128]">MP, OQMD, AFLOW are live. JARVIS, NOMAD, Materials Cloud, COD are next.</p>
              </div>
              <div className="rounded-[22px] border border-amber-100 bg-amber-50/70 p-4">
                <p className="text-[10px] font-bold uppercase tracking-widest text-amber-700">Closed data path</p>
                <p className="mt-2 text-sm font-bold text-[#0A1128]">For THERPRO, Gen IV Handbook, TAF-ID, ICSD, and CSD, the legal route is license or formal access only.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="mb-6 grid gap-6 lg:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
          <div className="rounded-[28px] border border-white/70 bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.05)] ring-1 ring-black/5">
            ...
            <div className="flex gap-3">
              <div className="relative flex-1">
                <input
                  ...
                  placeholder="Try UO2, U3Si2, UN, SiC, ZrO2, B4C, Li2TiO3..."
            ...
            <div className="mt-4 rounded-[20px] border border-indigo-100 bg-indigo-50/50 p-4">
              <div className="flex flex-wrap items-center gap-2 text-[11px] text-indigo-700">
                <Sparkles size={12} />
                <span className="font-semibold">Next public structure wave:</span>
                <span>JARVIS</span>
                <span>·</span>
                <span>NOMAD</span>
                <span>·</span>
                <span>Materials Cloud</span>
                <span>·</span>
                <span>COD</span>
              </div>
            </div>
          ...
          <div className="grid gap-4">
            <div className="rounded-[28px] border border-white/70 bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.05)] ring-1 ring-black/5">
              <p className="text-[10px] font-bold uppercase tracking-widest text-[#0A1128]">How we get non-open data</p>
              <div className="mt-4 space-y-3 text-[13px] leading-relaxed text-gray-500">
                ...
                <p><span className="font-semibold text-gray-700">2.</span> Apply through the official portal, member-country path, or NDA process.</p>
                <p><span className="font-semibold text-gray-700">3.</span> Ask for redistribution or mirror rights before shipping data inside the product.</p>
            ...
            <div className="rounded-[28px] border border-white/70 bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.05)] ring-1 ring-black/5">
              ...
              <div className="mt-4 space-y-3">
                <div className={`rounded-[18px] border p-4 ${TIER_META.open.panel}`}>
                  ...
                  <p className="mt-1 text-[11px] text-gray-500">Best first targets for in-product search, CIF fetch, or OPTIMADE aggregation.</p>
                </div>
                <div className={`rounded-[18px] border p-4 ${TIER_META.controlled.panel}`}>
                  <p className="text-xs font-bold text-[#0A1128]">Controlled</p>
                  <p className="mt-1 text-[11px] text-gray-500">Use the official access route first, then integrate only what redistribution permits.</p>
                </div>
                <div className={`rounded-[18px] border p-4 ${TIER_META.commercial.panel}`}>
                  <p className="text-xs font-bold text-[#0A1128]">Commercial</p>
                  <p className="mt-1 text-[11px] text-gray-500">Treat as enterprise connectors backed by customer-owned credentials or licensed local mirrors.</p>
                </div>
              </div>
            </div>
          </div>
            ...
            {isSearching ? (
              ...
              <div className="grid gap-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
                <div className="rounded-[24px] border border-gray-100 bg-[#FAFAF8] p-6">
                  ...
                  <h3 className="text-base font-black text-[#0A1128]">Start with a reactor or fuel formula</h3>
                  <p className="mt-2 max-w-xl text-sm leading-relaxed text-gray-500">
                    This panel is for structure-level discovery. It is best for things we can normalize as formulas,
                    crystal systems, hull energies, and crystal entries before pushing a candidate into Modeling.
                  </p>
                  <button
                    onClick={() => {
                      setQuery('UO2');
                  ...
                  >
                    Try UO2 <ArrowRight size={14} />
                  </button>
                </div>

                <div className="rounded-[24px] border border-gray-100 bg-[#FAFAF8] p-6">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-indigo-500">Why split the explorer?</p>
                  <p className="mt-3 text-sm leading-relaxed text-gray-500">
                    Structure databases plug into search cleanly. Property, irradiation, and nuclear-data libraries
                    usually need a metadata view first because their access model, identifiers, and files are very different.
                  </p>
                </div>
              </div>
            )}
        ...
        <section className="rounded-[28px] border border-white/70 bg-white p-6 shadow-[0_10px_40px_rgba(15,23,42,0.05)] ring-1 ring-black/5">
          <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-1 text-lg font-black text-[#0A1128]">Official nuclear materials source registry</h2>
              <p className="mt-1 text-xs leading-relaxed text-gray-500">
                Filter by access model and data family. The point here is to separate plug-in candidates from databases
                that need a formal access conversation first.
              </p>
            </div>
            <div className="rounded-full border border-gray-200 bg-gray-50 px-3 py-1 text-[10px] font-semibold text-gray-500">
