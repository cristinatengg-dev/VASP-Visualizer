...
def line_mode_kpoints(structure: Structure, line_density: int) -> str:
    ...
    lines = ["Band path generated by EliangMat AI", str(line_density), "Line-mode", "Reciprocal"]
...
def format_pdb(atoms: List[Dict[str, Any]], lattice: List[List[float]], title: str = "EliangMat AI structure") -> str:
...
def generate_graphite_lammps_data(params: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
    ...
    header = [
        "LAMMPS data file: AB-stacked graphite generated by EliangMat AI",
        "",
        f"{len(atoms)} atoms",
        "1 atom types",
        "",
...
def build_lammps_irradiation_input(params: Dict[str, Any]) -> str:
    ...
    return f"""# EliangMat AI LAMMPS template: graphite tensile-load irradiation creep
# Method mapping:
...
EliangMat AI {workflow_job_name(workflow)} for {structure['formula']}
...
def compile_abinit_inputs(request_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    abi = f"""# EliangMat AI ABINIT input
ndtset 1
...
def compile_gromacs_inputs(request_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    settings = quality_settings(quality)
    elements = unique_elements(structure["atoms"])
    gro_lines = ["EliangMat AI generated structure", f"{len(structure['atoms']):5d}"]
    for atom in structure["atoms"]:
        gro_lines.append(
    ...
    top = f"""; EliangMat AI generic GROMACS topology
[ defaults ]
...
def compile_namd_inputs(request_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    psf = "\n".join([
        ...
        "",
        "       1 !NTITLE",
        " REMARKS EliangMat AI generated PSF",
        "",
    ...
    prm = "\n".join([
        "* EliangMat AI generic NAMD parameter starter",
        "*",
        *masses,
        "",
...
"""
    ...
    mdin = f"""EliangMat AI AMBER starter
&cntrl
  imin={imin},
  maxcyc=2000,
  ncyc=1000,
