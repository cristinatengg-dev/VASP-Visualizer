...
const { runRetrievalAgentStream } = require('../../retrieval/agent');
const {
  SessionModel,
  ArtifactModel,
  TaskRunModel,
...
async function runRetrievalAgentOnce(userPrompt) {
  const stageEvents = [];
  let completion = null;
  let errorMessage = null;

  await runRetrievalAgentStream(userPrompt, (chunk) => {
    let parsed;
    try {
      parsed = JSON.parse(String(chunk || '').trim());
    } catch (_error) {
      return;
    }

    if (parsed.type === 'stage') {
      stageEvents.push(parsed);
      return;
    }

    if (parsed.type === 'complete') {
      completion = parsed.data || null;
      return;
    }

    if (parsed.type === 'error') {
      errorMessage = parsed.content || 'Retrieval agent failed';
    }
  });

  if (!completion) {
    throw new Error(errorMessage || 'Retrieval agent did not return a completed payload');
  }
...
function buildRetrievalGoalPrompt(prompt) {
  const normalized = String(prompt || '').trim().replace(/\s+/g, ' ');
  return normalized || 'Discover literature-grounded materials ideas';
}
...
function createRuntimeDemoRouter() {
  ...
  router.post('/retrieval/discover', async (req, res) => {
    try {
      await connectRuntimeDb();
      await runtimeCore.skillService.ensureBuiltinSkills();
      ...
      const firstStep = buildFirstStep(req.body, {
        firstStepId: `retrieval-discover-${Date.now()}`,
        firstSkillId: 'retrieval_discover',
        firstAgentId: 'retrieval-subagent',
      });
      ...
      if (sessionId) {
        replanned = await runtimeCore.replanSession({
          sessionId,
          replanReason: String(req.body.replanReason || 'retrieval_discover').trim(),
          goalArtifact: {
            status: 'ready',
            summary: buildRetrievalGoalPrompt(prompt),
            preview: {
              prompt,
              source: 'retrieval.discover',
            },
            payloadType: 'json',
            approvalStatus: 'none',
            riskLevel: 'low',
          ...
          planArtifact: {
            status: 'ready',
            summary: `Executable retrieval plan for: ${buildRetrievalGoalPrompt(prompt)}`,
            preview: buildDefaultPlanSpec(buildRetrievalGoalPrompt(prompt), firstStep),
            payloadType: 'json',
            approvalStatus: 'none',
            riskLevel: 'low',
          },
        ...
        taskRun = replanned.firstTaskRun;
      } else {
        const seeded = await runtimeCore.submitGoalAndCreatePlan({
          ownerId,
          projectId,
      ...
      try {
        const retrieval = await runRetrievalAgentOnce(prompt);
        const completion = retrieval.completion;
        const referenceIdentity = runtimeCore.artifactService.reserveArtifactIdentity();
        const ideaIdentity = runtimeCore.artifactService.reserveArtifactIdentity();

        const referencePayload = {
          prompt,
          summary: completion.summary,
          userGoal: completion.user_goal,
          papers: Array.isArray(completion.papers) ? completion.papers : [],
          structures: Array.isArray(completion.structures) ? completion.structures : [],
          stageTimeline: retrieval.stageEvents,
        };
        ...
        const ideaPayload = {
          ...
          summary: completion.summary,
          userGoal: completion.user_goal,
          ideaCards: Array.isArray(completion.idea_cards) ? completion.idea_cards : [],
          recommendedIdeaId: completion.recommended_idea_id || null,
          handoff: completion.handoff || null,
          sourceReferenceArtifactId: referenceIdentity._id,
        };

        const [storedReference, storedIdea] = await Promise.all([
          runtimeCore.artifactStorage.materializeJsonPayload({
            artifactId: referenceIdentity._id,
            lineageRootId: referenceIdentity.lineageRootId,
            version: referenceIdentity.version,
            payload: referencePayload,
          }),
          runtimeCore.artifactStorage.materializeJsonPayload({
            artifactId: ideaIdentity._id,
            lineageRootId: ideaIdentity.lineageRootId,
            version: ideaIdentity.version,
            payload: ideaPayload,
          }),
        ]);

        const finalized = await runtimeCore.withTransaction(async (tx) => {
          const referenceArtifact = await runtimeCore.artifactService.createArtifact(
            {
              ...referenceIdentity,
              kind: 'reference_bundle',
              sessionId: session._id,
              projectId: session.projectId,
              sourceArtifacts: Array.from(new Set([goalArtifact._id, planArtifact._id])),
              producedByTaskRun: taskRun._id,
              ...
              approvalStatus: 'none',
              isConsumable: true,
              payloadRef: storedReference.payloadRef,
              payloadType: storedReference.payloadType,
              mimeType: storedReference.mimeType,
              blobSizeBytes: storedReference.blobSizeBytes,
              contentHash: storedReference.contentHash,
              summary: `Reference bundle · ${referencePayload.papers.length} papers · ${referencePayload.structures.length} structures`,
              preview: {
                artifactType: 'reference_bundle',
                prompt,
                paperCount: referencePayload.papers.length,
                structureCount: referencePayload.structures.length,
              },
            },
            tx
          );

          const ideaArtifact = await runtimeCore.artifactService.createArtifact(
            {
              ...ideaIdentity,
              kind: 'idea_shortlist',
              sessionId: session._id,
              projectId: session.projectId,
              sourceArtifacts: Array.from(new Set([goalArtifact._id, planArtifact._id, referenceArtifact._id])),
              producedByTaskRun: taskRun._id,
              producedBySkill: taskRun.skillId,
              status: 'ready',
              lifecycleStage: 'validated',
              ...
              approvalStatus: 'none',
              isConsumable: true,
              payloadRef: storedIdea.payloadRef,
              payloadType: storedIdea.payloadType,
              mimeType: storedIdea.mimeType,
              blobSizeBytes: storedIdea.blobSizeBytes,
              contentHash: storedIdea.contentHash,
              summary: completion.handoff?.idea_title
                ? `Idea shortlist · recommended ${completion.handoff.idea_title}`
                : `Idea shortlist · ${ideaPayload.ideaCards.length} ideas`,
              preview: {
                artifactType: 'idea_shortlist',
                prompt,
                ideaCount: ideaPayload.ideaCards.length,
                recommendedIdeaId: ideaPayload.recommendedIdeaId,
                recommendedTitle: completion.handoff?.idea_title || null,
                formula: completion.handoff?.formula || null,
                materialId: completion.handoff?.material_id || null,
              },
            },
            tx
          );
          ...
          await runtimeCore.taskRunService.appendOutputArtifacts({
            taskRunId: taskRun._id,
            artifactIds: [referenceArtifact._id, ideaArtifact._id],
            tx,
          });
            ...
            [
              {
                ...
                taskRunId: taskRun._id,
                category: 'domain',
                type: 'artifact.reference_bundle.created',
                producerType: 'subagent',
                correlationId: taskRun.correlationId,
                payload: {
                  artifactId: referenceArtifact._id,
                  paperCount: referencePayload.papers.length,
                  structureCount: referencePayload.structures.length,
                },
              },
              {
                ...
                taskRunId: taskRun._id,
                category: 'domain',
                type: 'artifact.idea_shortlist.created',
                producerType: 'subagent',
                correlationId: taskRun.correlationId,
                payload: {
                  artifactId: ideaArtifact._id,
                  recommendedIdeaId: ideaPayload.recommendedIdeaId,
                  recommendedTitle: completion.handoff?.idea_title || null,
                },
              },
              {
                ...
                payload: {
                  taskRunId: taskRun._id,
                  outputArtifactIds: [referenceArtifact._id, ideaArtifact._id],
                },
              },
            ],
            tx
        ...
        return res.status(201).json({
          ...
          success: true,
          sessionId: session._id,
          goalArtifactId: goalArtifact._id,
          planArtifactId: planArtifact._id,
          taskRunId: finalized.taskRun._id,
          taskRunStatus: finalized.taskRun.status,
          referenceArtifactId: finalized.referenceArtifact._id,
          ideaArtifactId: finalized.ideaArtifact._id,
          previousGoalArtifactId: replanned?.previousGoalArtifact?._id || null,
          previousPlanArtifactId: replanned?.previousPlanArtifact?._id || null,
          result: completion,
        });
      } catch (error) {
        await runtimeCore.withTransaction(async (tx) => {
          await runtimeCore.taskRunService.transitionTaskRun({
            ...
            toStatus: 'failed',
            patch: {
              terminalReason: 'retrieval_discover_failed',
            },
            tx,
          });
  ...
  router.post('/workspace/report', async (req, res) => {
    try {
      await connectRuntimeDb();
      await runtimeCore.skillService.ensureBuiltinSkills();
      ...
      const { session, goalArtifact, planArtifact } = loaded;
      const firstStep = buildFirstStep(req.body, {
        firstStepId: `workspace-report-${Date.now()}`,
        firstSkillId: 'compile_project_report',
        firstAgentId: 'report-subagent',
      });
      ...
      const taskRun = await runtimeCore.withTransaction(async (tx) =>
        runtimeCore.createAndStageTaskRun({
          ...
          taskSpec: {
            ...firstStep,
            inputArtifacts: Array.from(new Set([goalArtifact._id, planArtifact._id])),
            retryable: true,
          },
          tx,
        })
      ...
      const latestArtifacts = {
        ideaShortlist: summarizeLatestArtifact(findLatest('idea_shortlist')),
        referenceBundle: summarizeLatestArtifact(findLatest('reference_bundle')),
        structure: summarizeLatestArtifact(findLatest('structure')),
        computeInputSet: summarizeLatestArtifact(findLatest('compute_input_set')),
        resultBundle: summarizeLatestArtifact(findLatest('result_bundle')),
        renderingReport: summarizeLatestArtifact(
          artifacts
            .filter((artifact) => artifact.kind === 'report' && artifact.preview && artifact.preview.structured)
            .slice(-1)[0] || null
        ),
        compiledPrompt: summarizeLatestArtifact(
          artifacts
            .filter((artifact) => artifact.kind === 'report' && artifact.preview && artifact.preview.compiledPrompt)
            .slice(-1)[0] || null
        ),
        visualAsset: summarizeLatestArtifact(findLatest('visual_asset')),
      };
      ...
      const reportPayload = {
        ...
        goal: {
          summary: goalArtifact.summary || null,
          preview: goalArtifact.preview || {},
        },
        activePlan: {
          summary: planArtifact.summary || null,
        ...
        summary: {
          ...
          completedJobs: jobRuns.filter((job) => job.status === 'succeeded').length,
        },
        latestArtifacts,
        notes: notes || null,
        nextActions: [
          latestArtifacts.ideaShortlist ? 'Review shortlisted ideas and confirm the recommended modeling path.' : 'Run the Idea step to create a literature-grounded shortlist.',
          latestArtifacts.structure ? 'Inspect the generated structure and refine the modeling parameters if needed.' : 'Build a validated structure artifact from the Modeling step.',
          latestArtifacts.computeInputSet ? 'Compile or submit the compute input set for execution.' : 'Generate a compute input set once the structure is ready.',
          latestArtifacts.visualAsset ? 'Review the latest visual asset and prepare the project report for sharing.' : 'Create a rendering brief or generate a visual asset for downstream reporting.',
        ],
        generatedAt: new Date().toISOString(),
      };

      const reportIdentity = runtimeCore.artifactService.reserveArtifactIdentity();
      const storedReport = await runtimeCore.artifactStorage.materializeJsonPayload({
        artifactId: reportIdentity._id,
        lineageRootId: reportIdentity.lineageRootId,
        version: reportIdentity.version,
        ...
        const reportArtifact = await runtimeCore.artifactService.createArtifact(
          {
            ...
            sessionId: session._id,
            projectId: session.projectId,
            sourceArtifacts: Array.from(new Set([
              goalArtifact._id,
              planArtifact._id,
              ...artifacts.slice(-8).map((artifact) => artifact._id),
            ])),
            producedByTaskRun: taskRun._id,
            producedBySkill: taskRun.skillId,
            status: 'ready',
            ...
            approvalStatus: 'none',
            isConsumable: true,
            payloadRef: storedReport.payloadRef,
            payloadType: storedReport.payloadType,
            mimeType: storedReport.mimeType,
            blobSizeBytes: storedReport.blobSizeBytes,
            contentHash: storedReport.contentHash,
            summary: `Project report · ${goalArtifact.summary || session._id}`,
            preview: {
              artifactType: 'project_report',
              sessionId: session._id,
              artifactCount: artifacts.length,
              taskRunCount: taskRuns.length,
              pendingApprovals: reportPayload.summary.pendingApprovals,
              latestVisualAssetId: latestArtifacts.visualAsset?.artifactId || null,
            },
          },
          tx
        );
          ...
          [
            {
              ...
              taskRunId: taskRun._id,
              category: 'domain',
              type: 'artifact.project_report.created',
              producerType: 'subagent',
              correlationId: taskRun.correlationId,
              payload: {
      ...
      return res.status(201).json({
        ...
        taskRunId: finalized.taskRun._id,
        taskRunStatus: finalized.taskRun.status,
        reportArtifactId: finalized.reportArtifact._id,
